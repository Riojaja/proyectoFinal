...
public class CategoriaController {
    ...
    public ResponseEntity<CategoriaResponseDTO> crear(@Valid @RequestBody CategoriaRequestDTO request) {
        if (categoriaService.existeNombre(request.getNombre())) {
            throw new RuntimeException("El nombre de esta categoría ya existe"); // Atrapado por GlobalExceptionHandler
        }
    ...
    @PutMapping("/{id}")
    public ResponseEntity<CategoriaResponseDTO> editar(@PathVariable Integer id, @Valid @RequestBody CategoriaRequestDTO request) {
        Categoria categoriaExistente = categoriaService.buscarPorId(id)
                .orElseThrow(() -> new RuntimeException("Categoría no encontrada con ID: " + id));

        if (!categoriaExistente.getNombre().equals(request.getNombre()) && categoriaService.existeNombre(request.getNombre())) {
        ...
        if (request.getIdCategoriaPadre() != null) {
            ...
            categoriaExistente.setCategoriaPadre(null); // Quitarle el padre
        }
