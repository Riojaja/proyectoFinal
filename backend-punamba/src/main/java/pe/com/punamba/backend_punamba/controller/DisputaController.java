...
public class DisputaController {
    ...
    public ResponseEntity<List<DisputaResponseDTO>> listarMisDisputas(Authentication auth) {
        ...
        return ResponseEntity.ok(disputaService.listarPorUsuario(usuarioLogueado.getIdUsuario()).stream()
                .map(disputaMapper::toResponse).collect(Collectors.toList()));
    }
    ...
    public ResponseEntity<?> abrirDisputa(@Valid @RequestBody DisputaRequestDTO request, Authentication auth) {
        ...
        try {
            ...
            Disputa nuevaDisputa = disputaService.abrirDisputa(usuarioLogueado.getIdUsuario(), request.getIdOrdenDetalle(), datosDisputa);
    ...
    public ResponseEntity<?> resolverDisputa(@PathVariable Integer id, @RequestBody Map<String, String> payload) {
        try {
            ...
            return ResponseEntity.ok(disputaMapper.toResponse(disputaService.resolverDisputa(id, nuevoEstado)));
