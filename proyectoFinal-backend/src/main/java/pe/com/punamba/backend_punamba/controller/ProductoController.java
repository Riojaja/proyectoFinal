...
public class ProductoController {
        ...
        public ResponseEntity<List<ProductoResponseDTO>> listar(
            ...
            if (q != null && !q.trim().isEmpty()) {
                // Búsqueda universal
                lista = productoService.buscarPorTermino(q, limit).stream()
                        .map(productoMapper::toResponse)
                        .collect(Collectors.toList());
            } else if (categoria != null) {
                lista = productoService.listarPorCategoria(categoria, limit).stream()
                        .map(productoMapper::toResponse)
                        .collect(Collectors.toList());
            } else if (tipo != null) {
                switch (tipo) {
                    case "nuevos":
                        lista = productoService.listarNuevos(limit).stream()
                                .map(productoMapper::toResponse)
                                .collect(Collectors.toList());
                        break;
                    case "ofertas":
                        lista = productoService.listarOfertas(limit).stream()
                                .map(productoMapper::toResponse)
                                .collect(Collectors.toList());
                        break;
                    default: // mas-vendidos
                        lista = productoService.listarMasVendidos(limit).stream()
                                .map(productoMapper::toResponse)
                                .collect(Collectors.toList());
                }
            } else {
                lista = productoService.listarHome(null, null, limit).stream()
                        .map(productoMapper::toResponse)
                        .collect(Collectors.toList());
            }
