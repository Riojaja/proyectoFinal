...
public class AuthService {
    ...
    public AuthResponse register(RegisterRequest request) {
        ...
        String nombreRolSolicitado = (request.getRol() != null && !request.getRol().trim().isEmpty()) 
                                    ? request.getRol().trim().toUpperCase() 
                                    : "CLIENTE";

        Rol rolAsignado = rolRepository.findByNombre(nombreRolSolicitado)
                .orElseThrow(() -> new RuntimeException("Error: El rol " + nombreRolSolicitado + " no existe."));
        
        usuario.getRoles().add(rolAsignado);
        
        Usuario usuarioGuardado = repository.saveAndFlush(usuario);
        
        if ("VENDEDOR".equals(nombreRolSolicitado)) {
            Vendedor vendedor = new Vendedor();
            vendedor.setUsuario(usuarioGuardado); 
            vendedor.setNombreTienda("Tienda de " + usuarioGuardado.getNombre());
            vendedor.setRuc(usuarioGuardado.getNumeroDocumento());
