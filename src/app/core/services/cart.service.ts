...
export class CartService {
  ...
  removePurchasedItems(itemsComprados: CartItem[]) {
    const idsComprados = new Set(
      (itemsComprados || []).map(x => Number(x.idVariante))
    );

    if (idsComprados.size === 0) return;

    if (this.auth.isLogged()) {
      const actuales = this.getItems();
      const restantes = actuales.filter(x => !idsComprados.has(Number(x.idVariante)));
      this.subject.next(restantes);
      return;
    }

    const restantes = this.readGuest().filter(x => !idsComprados.has(Number(x.idVariante)));
    this.saveGuest(restantes);
  }
}
